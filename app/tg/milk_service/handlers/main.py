...
from app.tg.milk_service.keyboard import not_mercury_auth_data_kb, main_kb
...
async def milk_service_handler(message: Message, user: UserSchema, db: Database, scheduler: SchedulerRepo):
    if user.mercury_auth_data is None:
        ...
        msg = await message.answer(
            ...
            reply_markup=main_kb(is_schedule=bool(scheduler.get_user_jobs(message.from_user.id)))
        )
